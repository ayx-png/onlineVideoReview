...
public class RtcTokenBuilder {
    ...
    public String buildTokenWithUserAccount(String appId, String appCertificate, 
    	...
    	agoraMedia.AccessToken builder = new agoraMedia.AccessToken(appId, appCertificate, channelName, account);
    	builder.addPrivilege(agoraMedia.AccessToken.Privileges.kJoinChannel, privilegeTs);
    	if (role == Role.Role_Publisher || role == Role.Role_Subscriber || role == Role.Role_Admin) {
    		builder.addPrivilege(agoraMedia.AccessToken.Privileges.kPublishAudioStream, privilegeTs);
    		builder.addPrivilege(agoraMedia.AccessToken.Privileges.kPublishVideoStream, privilegeTs);
    		builder.addPrivilege(agoraMedia.AccessToken.Privileges.kPublishDataStream, privilegeTs);
    	}
